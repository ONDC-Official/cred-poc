...
// Digio — CredentialService.ProcessCredentialRequest calls VerifyIdentity
// directly rather than resolving its own handler and calling Process,
// so there is exactly one code path that talks to Digio for a single
// credential, not two that happen to share a helper.
...
// shared DigioVerifier pipeline. credData is the same {"id_no","name","dob"}
...
//
// Returns the full VerificationResult, not just Digio's raw response —
